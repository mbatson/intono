...
The point of this style of todo note is to enable the user to record notes or reminders relating to the immediate context of what they are writing, without breaking the writing flow to open another document, and in a form so that the notes remain embedded within draft document and their original context, instead of recorded elsewhere in a place where they may be forgotten or lost.
...
While inline todo notes were introduced as having two elements (=((TODO: ​= and =))=), there are really three elements that make up the syntax of inline todo notes: the starting delimiter, an optional keyword, and the ending delimiter. The content of the note goes between the keyword (or the starting delimiter if there is no keyword set) and the ending delimiter. All three elements can be customised by the user.
